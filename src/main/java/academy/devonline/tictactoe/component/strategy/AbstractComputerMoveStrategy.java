...
public abstract class AbstractComputerMoveStrategy implements ComputerMoveStrategy {
    ...
    private boolean tryToMakeMoveByRows(GameTable gameTable, final Sign findSign, Sign moveSign) {
        for (int i = 0; i < 3; i++) {
    ...
    private boolean tryToMakeMoveByCols(GameTable gameTable, final Sign findSign, Sign moveSign) {
        for (int i = 0; i < 3; i++) {
    ...
    private boolean tryToMakeMoveByMainDiagonal(GameTable gameTable, final Sign findSign, Sign moveSign) {
        return tryToMakeMoveUsingLambdaConversion(gameTable, findSign, moveSign, -1, (k, j) -> new Cell(j, j));
    }

    private boolean tryToMakeMoveBySecondaryDiagonal(final GameTable gameTable, final Sign findSign, Sign moveSign) {
